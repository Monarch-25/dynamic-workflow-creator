...
Workflow: smoke_todo_semantic (1.0.0)
"""
...
WORKFLOW_NAME = "smoke_todo_semantic"
WORKFLOW_DESCRIPTION = "Search python files for TODO comments and summarize the findings"
SUBTASKS: List[Dict[str, str]] = [
  {
    "description": "Search python files for TODO comments and summarize the findings",
    "id": "search_python_files_todo_comments_summarize",
...
STEP_DEFS: List[Dict[str, Any]] = [
  {
    "config": {
      "subtask_description": "Search python files for TODO comments and summarize the findings",
      "tool_name": "tool_search_python_files_todo_comments_summarize"
    },
    "id": "search_python_files_todo_comments_summarize",
    "retry_policy": {
...
EDGE_DEFS: List[Dict[str, Any]] = [
  {
    "condition": null,
    "source": "search_python_files_todo_comments_summarize",
    "target": "synthesize"
  }
]
...
INTENT_SUMMARY: str = "Build a general-purpose workflow generator from natural language requirements. User context: Search python files for TODO comments and summarize the findings Execution path: 1. Parse requirements and lock the current task description in shared memory. 2. Split the task into independent subtasks for tool construction. 3. Build one tool function per subtask via tool-builder agents. 4. Verify each tool in a venv with execution-based integrity checks. 5. Iterate tool fix..."
CURRENT_TASK_DESCRIPTION: str = "User Requirements:\nSearch python files for TODO comments and summarize the findings\n\nApproved Plan:\n1. Parse requirements and lock the current task description in shared memory.\n2. Split the task into independent subtasks for tool construction.\n3. Build one tool function per subtask via tool-builder agents.\n4. Verify each tool in a venv with execution-based integrity checks.\n5. Iterate tool fixes until verifier passes or fallback tool is selected.\n6. Assemble a LangGraph workflow that runs subtasks and synthesizes results.\n7. Emit workflow folder with workflow.py, tools.py, README.md, and memory snapshot.\n8. Validate generated code and return user run instructions.\n\nIntent Summary:\nBuild a general-purpose workflow generator from natural language requirements. User context: Search python files for TODO comments and summarize the findings Execution path: 1. Parse requirements and lock the current task description in shared memory. 2. Split the task into independent subtasks for tool construction. 3. Build one tool function per subtask via tool-builder agents. 4. Verify each tool in a venv with execution-based integrity checks. 5. Iterate tool fix...\n"
