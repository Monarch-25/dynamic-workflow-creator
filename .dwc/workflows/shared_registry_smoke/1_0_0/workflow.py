...
Workflow: shared_registry_smoke (1.0.0)
"""
...
WORKFLOW_NAME = "shared_registry_smoke"
WORKFLOW_DESCRIPTION = "Create a workflow that searches python files and then summarizes findings for the user."
SUBTASKS: List[Dict[str, str]] = [
  {
    "description": "Create a workflow that searches python files",
    "id": "task_1",
    "tool_name": "tool_task_1"
  },
  {
    "description": "summarizes findings for the user",
    "id": "task_2",
...
INTENT_SUMMARY: str = "Build a general-purpose workflow generator from natural language requirements. User context: Create a workflow that searches python files and then summarizes findings for the user. Execution path: 1. Parse requirements and lock the current task description in shared memory. 2. Split the task into independent subtasks for tool construction. 3. Build one tool function per subtask via tool-builder agents. 4. Verify each tool in a venv with execution-based integrity checks. 5. Iterate tool fix..."
CURRENT_TASK_DESCRIPTION: str = "User Requirements:\nCreate a workflow that searches python files and then summarizes findings for the user.\n\nApproved Plan:\n1. Parse requirements and lock the current task description in shared memory.\n2. Split the task into independent subtasks for tool construction.\n3. Build one tool function per subtask via tool-builder agents.\n4. Verify each tool in a venv with execution-based integrity checks.\n5. Iterate tool fixes until verifier passes or fallback tool is selected.\n6. Assemble a LangGraph workflow that runs subtasks and synthesizes results.\n7. Emit workflow folder with workflow.py, tools.py, README.md, and memory snapshot.\n8. Validate generated code and return user run instructions.\n\nIntent Summary:\nBuild a general-purpose workflow generator from natural language requirements. User context: Create a workflow that searches python files and then summarizes findings for the user. Execution path: 1. Parse requirements and lock the current task description in shared memory. 2. Split the task into independent subtasks for tool construction. 3. Build one tool function per subtask via tool-builder agents. 4. Verify each tool in a venv with execution-based integrity checks. 5. Iterate tool fix...\n"
