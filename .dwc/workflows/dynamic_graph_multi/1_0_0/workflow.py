...
Workflow: dynamic_graph_multi (1.0.0)
"""
...
WORKFLOW_NAME = "dynamic_graph_multi"
WORKFLOW_DESCRIPTION = "First search python functions then summarize results then produce final concise answer."
SUBTASKS: List[Dict[str, str]] = [
  {
    "description": "First search python functions",
    "id": "task_1",
  ...
  {
    "description": "produce final concise answer",
    "id": "task_3",
...
STEP_DEFS: List[Dict[str, Any]] = [
  ...
  {
    "config": {
      "subtask_description": "First search python functions",
      "tool_name": "tool_task_1"
    },
    "id": "task_1",
    "retry_policy": {
  ...
  {
    "config": {
      "subtask_description": "produce final concise answer",
      "tool_name": "tool_task_3"
    },
    "id": "task_3",
    "retry_policy": {
...
INTENT_SUMMARY: str = "Build a general-purpose workflow generator from natural language requirements. User context: First search python functions then summarize results then produce final concise answer. Execution path: 1. Parse requirements and lock the current task description in shared memory. 2. Split the task into independent subtasks for tool construction. 3. Build one tool function per subtask via tool-builder agents. 4. Verify each tool in a venv with execution-based integrity checks. 5. Iterate tool fix..."
CURRENT_TASK_DESCRIPTION: str = "User Requirements:\nFirst search python functions then summarize results then produce final concise answer.\n\nApproved Plan:\n1. Parse requirements and lock the current task description in shared memory.\n2. Split the task into independent subtasks for tool construction.\n3. Build one tool function per subtask via tool-builder agents.\n4. Verify each tool in a venv with execution-based integrity checks.\n5. Iterate tool fixes until verifier passes or fallback tool is selected.\n6. Assemble a LangGraph workflow that runs subtasks and synthesizes results.\n7. Emit workflow folder with workflow.py, tools.py, README.md, and memory snapshot.\n8. Validate generated code and return user run instructions.\n\nIntent Summary:\nBuild a general-purpose workflow generator from natural language requirements. User context: First search python functions then summarize results then produce final concise answer. Execution path: 1. Parse requirements and lock the current task description in shared memory. 2. Split the task into independent subtasks for tool construction. 3. Build one tool function per subtask via tool-builder agents. 4. Verify each tool in a venv with execution-based integrity checks. 5. Iterate tool fix...\n"
