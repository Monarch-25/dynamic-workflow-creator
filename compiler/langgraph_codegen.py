...
"""
    ...
    def render_workflow_script(
        ...
        *,
        ...
        payload_subtasks = json.dumps(subtasks, indent=2, sort_keys=True)
        payload_extensions = json.dumps(io_contract.supported_doc_extensions)
        payload_prompt = json.dumps(synthesis_prompt)
        payload_plan = json.dumps(approved_plan)
        payload_intent = json.dumps(intent_summary)
        payload_task = json.dumps(current_task_description)
        payload_model = json.dumps(DWC_BEDROCK_MODEL_ID)
...
DOC_REQUIRED: bool = {json.dumps(io_contract.requires_document)}
